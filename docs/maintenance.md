...
| `.github/workflows/watch.yml` | Daily cron. Reads the resolver and: **builds** any kept version (ACTIVE or FROZEN) missing an image - new releases plus the one-time backfill of a maintained line's older patches (throttled by `max_dispatch`, default 12/run); **rebuilds** on edition/arch drift (ACTIVE only); **re-stacks** the ACTIVE set when a base digest changes. The DHI and Wolfi bases are watched **independently** (a DHI change re-stacks DHI only; Wolfi -> Wolfi). Buildable versions are proper-semver git tags that also have a published gateway image (not GitHub Releases, which are inconsistent). Manual `build.yml` dispatch still allows any version, including release candidates. |
