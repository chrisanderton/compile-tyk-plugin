...
| `.github/workflows/watch.yml` | Daily cron with two triggers: a new Gateway release builds it; a new base digest re-stacks all maintained versions on the fresh base. Auto-builds proper releases only (`prerelease==false` and strict `vX.Y.Z`); manual dispatch allows any version, including release candidates. |
