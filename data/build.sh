...
# Reconcile the plugin's go.mod with THIS Gateway's pinned Go BEFORE any `go` command reads it
# (incl. ensureGoMod and the build). The plugin must build with the Gateway's EXACT Go, and an old
# Gateway toolchain (e.g. go1.16 for v5.0.x) cannot even PARSE a newer go.mod - a 3-component
# `go 1.21.1` directive or a `toolchain` directive (both Go 1.21+) make every `go` subcommand fail
# to read the file ("invalid go version '1.21.1': must match format 1.23"). NB go1.16.15 is the
# TOOLCHAIN version; the go.mod `go` DIRECTIVE was major.minor-only until Go 1.21 - that is the field
# rejected. We DON'T silently rewrite the caller's declared Go - changing the directive can shift
# language semantics (Go 1.22 loop-variable scoping) and GODEBUG defaults. STRICT: any mismatch FAILS
# with actionable guidance unless OVERRIDE_PLUGIN_GO=1 is set, which opts in to a TEXTUAL rewrite (sed,
# since `go mod edit` can't read an unparseable file). Up-conversion is warned about extra loudly.
...
if [ -n "$GW_GO_MM" ] && [ -f go.mod ]; then
	...
	has_toolchain=false
	if grep -qE '^toolchain[[:space:]]' go.mod; then has_toolchain=true; fi

	if [ -z "$cur" ] && [ "$has_toolchain" = false ]; then
		# No 'go' directive at all - add the Gateway's (filling a default, not overriding a declaration).
		echo "INFO: plugin go.mod has no 'go' directive - adding go ${GW_GO_MM}"
		go mod edit -go="$GW_GO_MM"
	elif [ "$cur" != "$GW_GO_MM" ] || [ "$has_toolchain" = true ]; then
		dir=down
		if [ -n "$cur" ] && [ "$cur" != "$GW_GO_MM" ]; then
			[ "$(printf '%s\n%s\n' "$cur" "$GW_GO_MM" | sort -V | tail -1)" = "$cur" ] || dir=up
		fi
		why=""
		if [ "$cur" != "$GW_GO_MM" ]; then why="go directive '${cur:-<none>}' != Gateway Go ${GW_GO_MM}"; fi
		if [ "$has_toolchain" = true ]; then why="${why:+$why; }a 'toolchain' directive is present"; fi

		if [ "${OVERRIDE_PLUGIN_GO:-0}" != "1" ]; then
			echo "ERROR: plugin go.mod does not match the Gateway's Go (${GW_GO_MM}): ${why}." >&2
			echo "       Go plugins MUST be built with the Gateway's EXACT Go. Resolve it by either:" >&2
			echo "         (a) editing your plugin go.mod: set 'go ${GW_GO_MM}', remove any 'toolchain' line, and" >&2
			echo "             use no language/stdlib features newer than Go ${GW_GO_MM}; OR" >&2
			echo "         (b) re-running with  -e OVERRIDE_PLUGIN_GO=1  to let the compiler pin it automatically." >&2
			if [ "$dir" = up ]; then
				echo "       NOTE: (b) would UP-CONVERT go ${cur} -> ${GW_GO_MM}; go-version-gated behaviour (Go 1.22" >&2
				echo "             loop-variable scoping, GODEBUG defaults) can change. Review before overriding." >&2
			fi
			exit 1
		fi

		# OVERRIDE_PLUGIN_GO=1 - pin it, loudly.
		if [ "$dir" = up ]; then
			echo "WARN: ===== UP-CONVERTING plugin go ${cur} -> Gateway Go ${GW_GO_MM} (OVERRIDE_PLUGIN_GO=1) ====="
			echo "WARN:       go-version-gated behaviour (Go 1.22 loop variables, GODEBUG defaults) MAY change - verify your plugin."
		else
			echo "WARN: pinning plugin go.mod to Gateway Go ${GW_GO_MM} (OVERRIDE_PLUGIN_GO=1; plugin declared '${cur:-<none>}')."
		fi
		...
		if [ "$has_toolchain" = true ]; then sed -i -E '/^toolchain[[:space:]]/d' go.mod; fi
	fi
fi
