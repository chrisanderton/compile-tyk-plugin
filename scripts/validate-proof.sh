...
BASE_IMG="compile-tyk-plugin-base:proof"
...
export TYK_PLUGIN_SYSROOT_BASE=/opt/tyk/sysroots TYK_GLIBC_TARGET=2.17
...
rc=0
echo "--- GLIBC too new (native build vs target 2.17) ---"
EXPECT_GOARCH=arm64 MAX_GLIBC=2.17 GATEWAY_GO_VERSION=go1.25.10 /usr/local/bin/validate-plugin.sh /tmp/native.so || echo "PASS: rejected (exit $?)"
echo "--- wrong arch ---"
